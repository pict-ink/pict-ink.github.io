(() => {
	...
	async function runAIEffect(kind) {
		...
		try {
			...
			else { const layer=activeLayer(); layer.canvas=result; layer.offsetX=0; layer.offsetY=0; layer.opacity=1; layer.blend="source-over"; layer.name=(kind==="background"?"Background removed":kind[0].toUpperCase()+kind.slice(1))+" · "+layer.name; }
			asset.selection=null; pushHistory(before,"AI "+kind); syncControls(); render(); updateAIProgress({phase:"Finished",value:100,detail:"Applied to the document. Undo is available."}); showToast("AI effect applied");
		} catch(error) { console.error(error); const message=String(error?.message||error||"Unknown error"), detail=location.protocol==="file:"?"Serve Pict over http://localhost; browser AI cannot run from a file:// page.":message.includes("AI engine download failed")?"The AI runtime was blocked. Check content blockers or your connection, then reload Pict.":message.includes("fetch")||message.includes("network")?"The model download was interrupted. Check your connection or content blocker, then try again.":message.slice(0,180); updateAIProgress({phase:"Could not run effect",value:0,detail}); showToast("The local AI effect could not be completed"); }
