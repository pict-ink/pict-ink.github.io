(() => {
	...
	const state = {assets: [], active: -1, zoom: 0, history: [], future: [], tool: "select", gesture: null, selectionDraft: null, clipboard: null, colour: colours[0], restoring: false, textPoint: null, editingTextLayer: -1, hue: 345, nudge: null, cloneSource: null, grid: false, rulers: false, snap: true, spacePanning: false, panStart: null, correctionKind: null, draftTimer: null, hueAdjustment: null, hueFrame: 0};
	let toastTimer;
	...
	async function restoreSnapshot(saved) {
		...
		asset.layers = await Promise.all(saved.layers.map(async layer => ({...layer, canvas: await canvasFromURL(layer.data, asset.width, asset.height), mask: layer.mask ? await canvasFromURL(layer.mask, asset.width, asset.height) : null})));
	...
	function drawTransformedSource(context, layer, source) {
		const asset = activeAsset(), transform = layer.transform || {}, centreX = asset.width / 2, centreY = asset.height / 2; context.save(); context.translate(centreX + (layer.offsetX || 0), centreY + (layer.offsetY || 0)); context.rotate((transform.rotation || 0) * Math.PI / 180); context.transform(1, Math.tan((transform.skewY || 0) * Math.PI / 180), Math.tan((transform.skewX || 0) * Math.PI / 180), 1, 0, 0); context.scale(transform.scaleX || 1, transform.scaleY || 1); context.drawImage(source, -centreX, -centreY); context.restore();
	}
	...
	function compositeDocument(asset) {
		...
		for (const layer of asset.layers) if (layer.visible) {
			...
			const rendered = makeCanvas(asset.width, asset.height), renderedContext = rendered.getContext("2d"); drawTransformedSource(renderedContext, layer, layer.canvas); if (layer.mask) { const transformedMask = makeCanvas(asset.width, asset.height), maskContext = transformedMask.getContext("2d"); drawTransformedSource(maskContext, layer, layer.mask); renderedContext.globalCompositeOperation = "destination-in"; renderedContext.drawImage(transformedMask, 0, 0); }
	...
	function drawSelection(transient) {
		...
		if(activeAsset()?.guides?.length){context.save();context.strokeStyle="#80d7bdaa";context.lineWidth=1;context.setLineDash([4,3]);for(const guide of activeAsset().guides){context.beginPath();if(guide.axis==="x"){const p=sourceToCanvas({x:guide.value,y:activeAsset().edit.crop.y});context.moveTo(p.x,0);context.lineTo(p.x,interactionCanvas.height);}else{const p=sourceToCanvas({x:activeAsset().edit.crop.x,y:guide.value});context.moveTo(0,p.y);context.lineTo(interactionCanvas.width,p.y);}context.stroke();}context.restore();}
	...
	function morphMask(mask, radius, grow) { const width = mask.width, height = mask.height, source = mask.getContext("2d").getImageData(0, 0, width, height).data, horizontal = new Uint8Array(width * height), output = new Uint8Array(width * height), r = Math.max(1, Math.min(50, radius)); for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) { let value = grow ? 0 : 255; for (let px = Math.max(0, x-r); px <= Math.min(width-1, x+r); px++) value = grow ? Math.max(value, source[(y*width+px)*4+3]) : Math.min(value, source[(y*width+px)*4+3]); horizontal[y*width+x] = value; } for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) { let value = grow ? 0 : 255; for (let py = Math.max(0, y-r); py <= Math.min(height-1, y+r); py++) value = grow ? Math.max(value, horizontal[py*width+x]) : Math.min(value, horizontal[py*width+x]); output[y*width+x] = value; } const result = makeCanvas(width,height), image = result.getContext("2d").createImageData(width,height); for(let i=0;i<output.length;i++) image.data[i*4+3]=output[i]; result.getContext("2d").putImageData(image,0,0); return result; }
	...
	function applySelectionRefinement() { const asset=activeAsset(); if(!asset?.selection)return showToast("Make a selection first"); const before=snapshot(),operation=$("refineOperation").value,radius=Math.max(1,+$("refineRadius").value),original=selectionMask(asset.selection,0); let result=original; if(operation==="grow")result=morphMask(original,radius,true); if(operation==="shrink")result=morphMask(original,radius,false); if(operation==="smooth"){const blurred=makeCanvas(asset.width,asset.height),context=blurred.getContext("2d");context.filter=`blur(${Math.min(50,radius)}px)`;context.drawImage(original,0,0);result=blurred;} if(operation==="border"){const outer=morphMask(original,radius,true),inner=morphMask(original,radius,false),context=outer.getContext("2d");context.globalCompositeOperation="destination-out";context.drawImage(inner,0,0);result=outer;} if(operation==="islands")result=removeMaskIslands(original,Math.round(radius)); asset.selection=maskToSelection(result); pushHistory(before,"Refine selection"); $("refineDialog").close(); render(); }
	...
	function applyText() {
		...
		const asset = activeAsset(), before = snapshot(), spec = {text, x: point.x, y: point.y, font: $("textFont").value, size: Math.max(6, +$("textSize").value), weight: $("textBold").checked ? "700" : "400", style: $("textItalic").checked ? "italic" : "normal", align: $("textAlign").value, lineHeight: +$("textLineHeight").value, letterSpacing: +$("textLetterSpacing").value, colour: state.colour, opacity: +$("toolOpacity").value / 100}; let layer;
	...
	function pointerMove(event) {
		if(state.panStart){const stage=$("canvasStage");stage.scrollLeft=state.panStart.left-(event.clientX-state.panStart.x);stage.scrollTop=state.panStart.top-(event.clientY-state.panStart.y);return;} if (!state.gesture) { if (state.selectionDraft) { const local = pointerPosition(event); state.selectionDraft.cursor = snapPoint(canvasToSource(local.x, local.y)); drawSelection(); } return; } autoPanStage(event); const local = pointerPosition(event), rawPoint = canvasToSource(local.x, local.y), point = ["select","lasso","path","move","line","rectangle","ellipse","text"].includes(state.tool)?snapPoint(rawPoint):rawPoint, gesture = state.gesture, pressure=event.pointerType==="mouse"?1:Math.max(.05,event.pressure||gesture.pressure||.5);
	...
	function renderLayers() {
		const asset = activeAsset(); if (!asset) return; $("layersList").replaceChildren(...asset.layers.map((layer, index) => {
			...
			const opacity = document.createElement("button"); opacity.type = "button"; opacity.className = "layer-row-opacity layer-kind"; opacity.textContent = layer.type === "text" ? "T" : layer.type === "adjustment" ? "A" : layer.mask ? layer.maskEditing ? "M*" : "M" : Math.round(layer.opacity * 100) + "%"; opacity.title = layer.type === "text" ? "Edit text" : layer.type === "adjustment" ? "Adjustment layer" : layer.mask ? layer.maskEditing ? "Editing mask" : "Layer mask" : "Layer opacity"; if (layer.type === "text") opacity.addEventListener("click", event => { event.stopPropagation(); const spec = layer.textSpec; openTextDialog({x: spec.x, y: spec.y}, index); });
	...
	function addAdjustmentLayer() { if (!canAllocateLayer()) return; const asset = activeAsset(), before = snapshot(), kind = $("adjustmentType").value, amount = +$("adjustmentAmount").value, mask = asset.selection ? selectionMask(asset.selection, 0) : null; asset.layers.push({name: kind[0].toUpperCase() + kind.slice(1), type: "adjustment", canvas: makeCanvas(asset.width, asset.height), visible: true, opacity: 1, blend: "source-over", adjustment: {kind, amount}, mask}); asset.activeLayer = asset.layers.length - 1; asset.selection = null; pushHistory(before, "Add adjustment layer"); $("adjustmentDialog").close(); syncControls(); render(); }
	...
	function mergeVisibleLayers(flatten = false) { const asset = activeAsset(), before = snapshot(), composite = compositeDocument(asset), base = makeCanvas(asset.width, asset.height), context = base.getContext("2d"); if (flatten) { context.fillStyle = "#fff"; context.fillRect(0, 0, asset.width, asset.height); } context.drawImage(composite, 0, 0); asset.layers = [{name: flatten ? "Background" : "Merged visible", type: "pixel", canvas: base, visible: true, opacity: 1, blend: "source-over"}]; asset.activeLayer = 0; asset.selection = null; pushHistory(before, flatten ? "Flatten image" : "Merge visible"); syncControls(); render(); }
	function resizeCanvas() {
		...
		asset.layers = asset.layers.map(layer => { const resized = makeCanvas(width, height); resized.getContext("2d").drawImage(layer.canvas, (layer.offsetX || 0) + dx, (layer.offsetY || 0) + dy); let mask = null; if (layer.mask) { mask = makeCanvas(width, height); mask.getContext("2d").drawImage(layer.mask, dx, dy); } const textSpec = layer.textSpec ? {...layer.textSpec, x: layer.textSpec.x + dx, y: layer.textSpec.y + dy} : null; return {...layer, canvas: resized, mask, textSpec, offsetX: 0, offsetY: 0}; }); asset.guides = (asset.guides || []).map(guide => ({...guide, position: guide.position + (guide.axis === "x" ? dx : dy)})).filter(guide => guide.position >= 0 && guide.position <= (guide.axis === "x" ? width : height)); asset.width = width; asset.height = height; asset.edit.crop = {x: 0, y: 0, width, height}; asset.selection = null; pushHistory(before, "Resize canvas"); $("canvasDialog").close(); state.zoom = 0; syncControls(); render();
	}
	function applyLayerTransformValues(){const layer=activeLayer();if(!layer||layer.type==="adjustment")return showToast("Choose a pixel or text layer");const before=snapshot();layer.transform={...(layer.transform||{}),scaleX:Math.max(.01,+$("layerScaleX").value/100),scaleY:Math.max(.01,+$("layerScaleY").value/100),skewX:+$("layerSkewX").value,skewY:+$("layerSkewY").value,rotation:+$("layerRotation").value};pushHistory(before,"Transform layer");render();}
	...
	function applyCorrection(){const kind=state.correctionKind,asset=activeAsset(),layer=activeLayer();if(!kind||!layer||layer.type==="adjustment")return showToast("Choose a pixel or text layer");if(asset.width*asset.height>20000000&&["denoise","dust","lens"].includes(kind))return showToast("Resize below 20 MP for this memory-intensive correction");const before=snapshot(),width=layer.canvas.width,height=layer.canvas.height,sourceCanvas=makeCanvas(width,height),sourceContext=sourceCanvas.getContext("2d");sourceContext.drawImage(layer.canvas,0,0);const result=makeCanvas(width,height),context=result.getContext("2d"),amount=+$("correctionAmount").value,secondary=+$("correctionSecondary").value;context.drawImage(sourceCanvas,0,0);
	...
	async function loadProject(data) {
		if (data?.format !== "pict" || ![1, 2].includes(data.version) || !Array.isArray(data.assets)) throw new Error("Not a Pict project"); const assets = []; for (const saved of data.assets) { if (!saved.width || !saved.height || saved.width * saved.height > 40000000 || !Array.isArray(saved.layers) || !saved.layers.length || saved.width * saved.height * saved.layers.length > 96000000 || saved.layers.some(layer => typeof layer.data !== "string" || !layer.data.startsWith("data:image/png;base64,") || layer.mask && !layer.mask.startsWith("data:image/png;base64,"))) throw new Error("Unsafe project dimensions"); const layers = await Promise.all(saved.layers.map(async layer => ({...layer, type: layer.type || "pixel", canvas: await canvasFromURL(layer.data, saved.width, saved.height), mask: layer.mask ? await canvasFromURL(layer.mask, saved.width, saved.height) : null}))), composite = makeCanvas(saved.width, saved.height), compositeContext = composite.getContext("2d"); for (const layer of layers) if (layer.visible && layer.type !== "adjustment") { compositeContext.globalAlpha = layer.opacity; compositeContext.globalCompositeOperation = layer.blend || "source-over"; compositeContext.drawImage(layer.canvas, layer.offsetX || 0, layer.offsetY || 0); } const url = composite.toDataURL("image/png"), image = new Image(); image.src = url; await image.decode(); const file = new File([], saved.name || "project-image.png", {type: saved.type || "image/png"}); assets.push({file, image, url, width: saved.width, height: saved.height, edit: saved.edit, selection: saved.selection || null, guides: saved.guides || [], activeLayer: Math.min(saved.activeLayer || 0, layers.length - 1), layers}); }
	...
	$("horizontalRuler").addEventListener("click",event=>{const rect=interactionCanvas.getBoundingClientRect(),point=canvasToSource(event.clientX-rect.left,0);activeAsset().guides.push({axis:"x",position:point.x});render();}); $("verticalRuler").addEventListener("click",event=>{const rect=interactionCanvas.getBoundingClientRect(),point=canvasToSource(0,event.clientY-rect.top);activeAsset().guides.push({axis:"y",position:point.y});render();}); [$("horizontalRuler"),$("verticalRuler")].forEach(ruler=>ruler.addEventListener("dblclick",()=>{activeAsset().guides=[];render();showToast("Guides cleared");}));
